  ...
  return <header className={`fixed top-0 w-full z-50 transition-all ${scrolled ? 'bg-white/95 shadow-lg' : 'bg-white/90'} backdrop-blur-sm border-b border-gray-200`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
        <a href="#home" className="flex items-center gap-3">
          <img src="/MORECIVILFINALLOGOFORWEB.svg" alt="More Civil" className="h-30 w-auto -ml-10" />
        ...
        <button onClick={() => setOpen(v => !v)} className="md:hidden text-blue-900 p-2 border-8 border-black rounded-lg shadow-sm aspect-square flex items-center justify-center" aria-label="Toggle menu">
          Menu
        </button>
      </div>
